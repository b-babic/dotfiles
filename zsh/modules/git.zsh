...
alias guh='git reset origin/$(git_current_branch) --hard'
...
# List git tags sorted by most recent first
# Usage example: git_tags "v1.0"
function git_tags() {
  git for-each-ref --sort=creatordate --format='%(creatordate:short)_,,,_%(refname:strip=2)_,,,_%(subject)' refs/tags \
  | grep "$1" \
